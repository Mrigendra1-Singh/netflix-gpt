...
const Login = () => {
  ...
  .then((userCredential) => {
    ...
    updateProfile(user, {
        displayName: name.current.value, photoURL: "https://avatars.githubusercontent.com/u/99022947?v=4",
      })
      .then(() => {
  ...
  .catch((error) => {
   ...
   return (
       ...
       { !isSignInForm &&
           ( <input 
           ref={name}
           type="text" placeholder="Full  Name" className="p-4 my-4 w-full bg-gray-800"/>)}
           <input 
           ref={email}
